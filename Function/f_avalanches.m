function [avalanches] = f_avalanches(fingerprint, channel_length, time_length, timebinSize)
%% notes
%
%% input
%
...
%
%% main function
    % using the fingerprint construct the raster
    raster = zeros(channel_length, time_length);
    time = fingerprint.time;
    channel = fingerprint.node;
    for II = 1:length(time)
        raster(channel(II),time(II)) = 1;
    end
